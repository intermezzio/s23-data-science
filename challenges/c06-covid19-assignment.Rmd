...
- The counties with the highest cases per 100k have about 10x more cases per
100k than the average county at its peak cases per 100k.
- The counties with the highest deaths per 100k have more than 10x more deaths
than the average county at its peak deaths per 100k.
...
- There are a lot of counties that have different maximum cases per 100k than
...
- Counties in Massachusetts generally have higher max deaths per 100k than New
...
- There doesn't seem to be a correlation with population and cases or population
  and deaths, only a weak positive correlation between cases and deaths.

Part of this data sifting was figuring out how to parse county data from
Louisiana, which often would show up as blank because it has parishes instead.
To learn more about the data at hand, I filtered all Louisiana "county" names
and checked them here so I could update the parser in the previous merge.
```{r}
cases_and_deaths %>% 
  filter(
    region == "louisiana"
