...
- Which model is *most accurate*? Which is *least accurate*?
  - The cheating one was most accurate, and the nonphysical one was least
    accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  - avg_T is an output, which means it is a result of the exact same test that
    generates the T_norm value.
...
- What *Category* of variable is `idx`? Does it have any physical meaning?
  - This is metadata, meaning it has no influence on the test itself and is only
    for recordkeeping.
...
- Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
  - This is actually pretty hard to tell, since both seem loosely accurate.
    However, the one on the left (q4) appears slightly more accurate because
    the residuals seem smaller.
...
- How much do you trust your model? Why?
  - Because the model does not have a significant amount of data, especially in
    the range of parameters like `x` and `U_0` that are requested, this may not
    be the best model for predicting an interval.
- What kind of interval---confidence or prediction---would you use for this task, and why?
  - This is a prediction interval because I'm predicting a general distribution of values,
    not just a specific value.
...
- What interval for `T_norm` would you recommend the design team to plan around?
  - I would not be confident in recommending an interval for `T_norm` based on
    the lack of a conclusion from the analysis above.
- Are there any other recommendations you would provide?
  - I would recommend adding other parameters into the model or collecting more
    data that's similar to the preferred use case for modeling `T_norm`.
