...
- The bulk velocity is roughly around 1.5-2.5
- The mu_f and d_p values are tiny, the others aren’t
- I_0, N_p, and avg_q are very large
...
df_psaap %>% 
  ...
  facet_grid(~x) +
  theme_classic() +
  ggtitle("T norm for different channel locations") +
  no_x_axis
```
...
- Which model is *most accurate*? Which is *least accurate*?
  - The cheating one was most accurate, and the nonphysical one was
    least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is an output, which means it is a result of the exact same
    test that generates the T_norm value.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No, therefore even if it’s related, it can’t be a prediction.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - This is metadata, meaning it has no influence on the test itself and
    is only for recordkeeping.
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - This is actually pretty hard to tell, since both seem loosely
    accurate. However, the one on the left (q4) appears slightly more
    accurate because the residuals seem smaller.
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*?
  - Both have similarly large prediction intervals.
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - 1 for fit_simple, 2 for fit_q4
...
# )
# 
# df_validate_filt <- df_validate %>% filter(
#   x == df_design$x,
#   L == df_design$L,
#   W == df_design$W,
#   U_0 == df_design$U_0
...
- How much do you trust your model? Why?
  - Because the model does not have a significant amount of data,
    especially in the range of parameters like `x` and `U_0` that are
    requested, this may not be the best model for predicting an
    interval.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - This is a prediction interval because I’m predicting a general
    distribution of values, not just a specific value.
- What fraction of validation cases lie within the interval you predict?
  How does this compare with `pr_level`?
  - 3/60, which is way less than pr_level
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - I would not be confident in recommending an interval for `T_norm`
    based on the lack of a conclusion from the analysis above.
- Are there any other recommendations you would provide?
  - I would recommend adding other parameters into the model or
    collecting more data that’s similar to the preferred use case for
    modeling `T_norm`.
