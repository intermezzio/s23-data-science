...
df_q2 <-
  df_q1 %>% 
    summarize(
      ...
      epsilon_se = epsilon_sd / sqrt(length(epsilon_c))
    )
...
-   Is `epsilon_mean` large or small compared to `epsilon_sd`? Yes
-   Is `epsilon_mean` large or small compared to `c_michelson_uncertainty?`? Yes
...
C <- 0.99
c_meas_mean <- mean(df_michelson$c_meas)
df_q3 <- df_q2 %>% 
  mutate(
    c_lo = c_meas_mean + qnorm(0.005) * epsilon_se,
    c_hi = c_meas_mean + qnorm(0.995) * epsilon_se
  )
