...
**Observations**:
- There is no highway 27.
- Cities indexed with higher numbers have highways indexed with higher numbers.
...
**Observations**:
- The class attribute is repeated in both the x axis and color.
- There are more SUVs and compact cars, and fewer two seaters.
...
**Observations**:
- All 2 seaters are two wheel drive.
- Most SUVs and pickups are 4 wheel drive.
- Only the subcompact model has options for forward and rear wheel drive.

**Compare `v1` and `v2`**:

- V2 had more data to make observations on, primarily because v1 repeated data in color and x axis.
...
**Observations**
- Dodging in the bar plot allows it to split the data after it's put into bars.
...
```{r q5-task}
mpg %>%
  ggplot(aes(x = manufacturer, fill = as.character(cyl))) +
