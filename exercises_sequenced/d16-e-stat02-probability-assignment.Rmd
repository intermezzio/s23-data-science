...
df_z %>% 
  summarize(count_total = n(), count_A = sum(A), fr = count_A / count_total)
```

**Observations**:

- What value of `fr` did you predict?
  - I predicted 0.5 for fr.
- Did your prediction match your calculation?
  - The prediction was very close to the output of 0.52 from the simualtion.
...
- What value does `fr` appear to be limiting towards?
  - It appears to approach approximately 0.95.
...
- What value is `fr` approaching?
  - The value of `fr` approaches zero.
