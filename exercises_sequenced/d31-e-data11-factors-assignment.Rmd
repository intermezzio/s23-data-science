...
- How are the factor levels ordered? Fair to Ideal
- What---if any---relation does the order of the factor levels have to trends in the data? probably price
...
- How are the factor levels ordered? alphabetical
- What---if any---relation does the order of the factor levels have to trends in the data? none
...
gapminder %>%
  mutate(
    continent = fct_relevel(continent, "Americas", "Europe", "Africa", "Asia", "Oceania")
  )
```

**Observations**:

- How are the factor levels (re)ordered? West to East
...
*Before*
- Document your observations before sorting. Random order
*Sorted*
- Modify the code to sort `manufacturer`, then note any additional observations you make.
- Easier to read with more comparable stats
...
- First by the first factor, then by the second one
