...
- The rows are not aligned so they don't match.
...
Use `expand_grid` to create a `criteria` dataframe with the `month` equal to
`8, 9` and the airport identifiers in `dest` for the San Francisco, San Jose,
...
```{r q3-task}
criteria <-
  expand_grid(month = c(8, 9), dest = c("SFO", "OAK", "SJC"))
