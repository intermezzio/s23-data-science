...
df_sample %>% 
  summarize(
    ...
    price_sd = sd(price),
    price_se = price_sd / sqrt(length(price)),
    price_lo = price_mean - pnorm(0.025) * price_se,
    price_hi = price_mean + pnorm(0.025) * price_se
  )
# price_threshold
```
...
- Can you reject the null hypothesis **H_0**?
  - No, I cannot reject the null hypothesis.
- How does this affect your decision?
  - I would not buy the diamonds in bulk.
...
df_prices <-
  df_resample_norm %>% mutate(
    mean_est = map_dbl(
    df_resample_norm %>% pull(df),
    ~ summarize(.x, mean_est = mean(cut == "Premium" | cut == "Ideal")) %>% pull(mean_est)
    )
  ) %>% 
  summarize(
    perc_mean = mean(mean_est),
    perc_sd = sd(mean_est),
    perc_se = sd(mean_est) / sqrt(length(mean_est)),
    perc_hi = perc_mean + pnorm(0.005) * perc_se,
    perc_lo = perc_mean - pnorm(0.005) * perc_se
  )
...
df_population %>% 
  summarize(
    perc_good = mean(cut == "Premium" | cut == "Ideal"),
    price_mean = mean(price)
  )
```
