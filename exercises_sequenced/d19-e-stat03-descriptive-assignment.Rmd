...
- What is the mean departure delay time, among available values?
  - The mean departure delay time among available values is 12.56 minutes.
...
- Which carriers are at the top of both `mean` and `median` departure delay?
  - At the top of both lists, we can see FL, WN, and F9.
- Which carriers differ in the two lists?
  - The EV and YV carriers are only in mean and the UA and UX carriers are only
  in median.
- What do negative values of `dep_delay` mean?
  - Departures were earlier than anticipated.
...
- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
  - HA has the biggest difference - it may have a lot of outliers.
...
```{r ex-cor-line}
slope <- -0.1 # Play with this value; observe the correlation
df_line <-
...
- For what values of `slope` is the correlation positive?
  - Positive when the slope is positive
- For what values of `slope` is the correlation negative?
  - Negative when the slope is negative
...
- What do you predict the correlations to be?
  - I predicted the Pearson one to be zero and the Spearman one to be 1
- What are the actual correlations?
  - Both were about zero
