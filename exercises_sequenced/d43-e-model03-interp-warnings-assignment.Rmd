...
fit_lower <- 
  lm(
    data = diamonds_train %>% filter(grouping == "Lower carat"),
    formula = price ~ cut
  )
fit_upper <- 
  lm(
    data = diamonds_train %>% filter(grouping == "Upper carat"),
    formula = price ~ cut
  )
...
- What trends do the lower and upper models between `carat` and `cut`?
  - Similar to the limited datasets
...
- What might account for this behavior?
  ...
  - The ones with worse cuts have more carats
