...
fit_q5 <-
 lm(
   data = diamonds_train,
   formula = price ~ (carat + cut + color + clarity + table) # Add more variables!
 )
...
- What effect would it have to train on `diamonds_test_bad` and then predict on `diamonds_validate_bad`?
  - It would have different features, train wouldn't ideally converge on validate
