...
Notice that the `trans` column of `mpg` has many entries of the form `auto|manual\\([[:alpha:]]\\d\\)`; use `str_extract()` to create a new column `tmp` with just the code inside the parentheses extracting `[[:alpha:]]\\d`.
...
df_q6 <- df_q6 %>% 
  pivot_longer(
    ...
    names_transform = c(angle = as.integer)
  )
```
...
```{r q6-tests}
## NOTE: No need to change this (unless you really want to...)!
assertthat::assert_that(dim(df_q6)[1] == 8)
