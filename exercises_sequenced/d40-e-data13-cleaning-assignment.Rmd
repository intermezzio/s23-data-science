...
```{r look-no-names}
## NOTE: No need to edit; run and inspect
df_no_cols <- read_csv(filename_disease)
df_no_cols %>% glimpse()
```
...
df_q2 <- read_csv(filename_disease, col_names = col_names, col_types = "dddddddddddddd") # "iiiiiiiiididdi")
...
df_q4 <-
  df_q2 %>% mutate(
    ...
    cp = as.factor(convert_cp(cp)),
    restecg = as.factor(convert_restecg(restecg)),
    exang = convert_exang(exang),
    thal = as.factor(convert_thal(thal)),
    fbs = convert_fbs(fbs),
  )
df_q4
```
