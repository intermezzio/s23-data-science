...
- What kind of variable is `carat`? What is `cut`?
  Carat is a float and cut is ordinal
- Can we "multiply" `cut` by a constant? Why or why not?
  Technically if it's treated like an integer
- Describe the difference between the `term`s for `carat` and `cut`.
  one's continuous one's discrete
...
- Is your model more or less accurate than `fit_carat`? Less!
