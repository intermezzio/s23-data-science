...
df_price <- read_csv("data/beef_price.csv", col_names = c("month", "price_perc"))
df_production <- read_csv("data/beef_production.csv", col_names = c("month", "prod_perc"))
```
...
df_price %>% 
  mutate(
    approx_month = round(month, 2)
  ) %>% 
  full_join(
    df_production %>% mutate(approx_month = round(month, 2)),
    by = "approx_month"
  ) %>% 
  drop_na() %>% 
  # glimpse()
  # arrange(approx_month) %>% 
  ggplot(aes(x = price_perc, y = prod_perc, color = approx_month)) +
  geom_point()
```
