...
yippee_summary <- function (df, stat) {
  df %>% 
    summarize(
      stat_mean = mean({{stat}}),
      stat_sd = sd({{stat}}),
      stat_se = stat_sd / sqrt(length({{stat}})),
      ci_lwr = stat_mean - 1.96 * stat_se,
      ci_upr = stat_mean + 1.96 * stat_se,
      pi_lwr = stat_mean - 1.96 * stat_sd,
      pi_upr = stat_mean + 1.96 * stat_sd
    )
}
df_q2 <- diamonds_train %>% 
  yippee_summary(price)
df_q2
```
...
- Compare the intervals against the price observations; do the intervals and observations relate to each other like you would expect?
  - Not at all! Price isn't below zero
- Are the CI and PI working correctly? How would you check this?
  - No! They don't catch the most dense areas and the PI has area in the interval that definitely shouldn't be
...
diamonds_validate %>% 
  summarize(
    in_pi = mean(df_q2$pi_lwr < price & price < df_q2$pi_upr),
    in_ci = df_q2$ci_lwr < mean(price) & mean(price) < df_q2$ci_upr
  )
```

**Observations**:

- Does your CI include the mean price of diamonds? Yes
  - Do we expect the CI it to *always* include the true parameter? No
- Does your PI include the appropriate fraction of diamond prices? Yes
