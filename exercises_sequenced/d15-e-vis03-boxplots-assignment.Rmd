...
diamonds %>%
  ...
  mutate(carat_cut = cut_width(carat, width = 0.5)) %>% 
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```

**Observations**
- More carats generally lead to a higher price.
- The median price increases with carats between 0 and 2.5 carats.
- Diamonds of at least 0.75 carats can sell for close to the maximum price in
this dataset.
- There is a higher interquartile range for diamonds with an average number of
carats, as opposed to a very low or high amount of carats.
...
- Did the most common group of cut and color change?
  - For diamonds with at least 2 carats, the most common group changes from ideal
  to premium.
