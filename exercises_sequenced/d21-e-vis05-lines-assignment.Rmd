...
- What does a line plot assume about the data?
  - This assumes that the data on the x axis are continuous and that there is
  only one datapoint per x value.
- How does the `mpg` dataset violate this assumption?
  - There are multiple highway values for each display value.
...
- What can you tell based on the `Raw` data?
  - I can see finer details in the trendline like oscilations in unemployment rates
  throughout each decade.
- What can you tell based on the `Smoothed` data?
  - I can see a general trend towards a higher unemployment rate over the last
  few decades.
